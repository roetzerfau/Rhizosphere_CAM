...
parameters.tau_ode = dayinseconds *10; %12 * 60;
%g.numT = 10;
C_SVector = ones(g.numT, 1) * parameters.K_Cliquid *10;%0.1
N_SVector =  C_SVector ./ parameters.C_N_S;

C_BVector = zeros(g.numT, 1);
C_BVector(1) = parameters.minConC_B;
N_BVector = C_BVector ./ parameters.C_N_B;




MNsolidVector = zeros(g.numT, 1);
visualizeDataSub(g, C_BVector, 'concentrationVector', 'concentrationVector', 0);
numberoftstps = dayinseconds/parameters.tau_ode;
range = round( 5/(60*60) * parameters.tau_ode);

[ N_SVector, C_SVector, N_BVector, C_BVector,MNsolidVector] = ...
calculateMBSolid(g,parameters, N_SVector, C_SVector, N_BVector, C_BVector, bulkVector,  MNsolidVector);


for j = 1:100
T_C_N = tic; 
for i = 1:(dayinseconds/parameters.tau_ode)
 C_Nstep = tic;
[ N_SVector, C_SVector, N_BVector, C_BVector,MNsolidVector] = ...
calculateMBSolid(g,parameters, N_SVector, C_SVector, N_BVector, C_BVector, bulkVector,  MNsolidVector);
...
numel(find(C_BVector > 0))
visualizeDataSub(g, C_BVector, 'concentrationVector', 'concentrationVector', j);
