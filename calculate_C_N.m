function [bulkVector, MB_Vector, N_SVector, C_SVector, N_BVector, C_BVector ,C_MNVector, N_MNVector, MNVector, POMVector, C_POMconcVector, POMParticleList, POMageVector, CO2Vector,leakedNVector, C_EXTVector_t, C_PPlantVector,N_PPlantVector, C_PMNVector, N_PMNVector, sumleakedN_S, CUE, U,T,B,R ] = ...
    calculate_C_N(g, parameters, bulkVector, MB_Vector, N_SVector, C_SVector, N_BVector, C_BVector, C_MNVector, N_MNVector, MNVector, POMVector, C_POMconcVector, reactiveSurfaceVector, POMParticleList, POMageVector,CO2Vector, leakedNVector,outerRootBorderInd)
  ...
  [ N_SVector, C_SVector, N_BVector, C_BVector,C_MNVector, N_MNVector, CO2Vector, C_EXTVector] = ...
    ...
    sumC_S = sum(C_SVector);
     ...
     U = U + abs(sumC_S - sumC_S_before);
     R = R + (sumCO2 - sumCO2_before);
     T = T + (sumC_MN - sumC_MN_before);
     B = B + (sumC_B - sumC_B_before);
