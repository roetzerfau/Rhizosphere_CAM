...
NX          = NN;                       % Number of horicontal Elements
NZd         = NN;                       % Number of vertical Elements

tau         = 1;                        % End-Time
epsilon     = 1;                        % Everything < epsilon is defined as zero

p           = 0;                        % Order of Polynomials for LDG Disc.
ord         = 2;                        % Order of Gaussian Integration Rule
...
%% Creating Initial Distribution of Bulk and Biomass                        %% Can be changed
 bulkVector_      = randi([0,1], g.numTsub, 1); % 1 is bulk, 0 is air     % Vector containing Bulk Distribution
...
%L = ichol(sysA); bicgstabl
%[sysXbac, flag,relres,iter,resvec,lsvec] = lsqr(sysA ,sysVbac,1e-6,1000);
%issymmetric(sysA)
%sysXbac = gmres(sysA ,sysVbac, [],1e-5,1000);%,
%sysXbac = lsqr(sysA ,sysVbac,1e-6,10000);
%sysXbac = bicgstabl(sysA ,sysVbac,1e-6,10000);
%sysXbac = lsqminnorm(sysA ,sysVbac);
...
ubacLagr                   = projectDG2LagrangeSub(uDGbac);
visualizeDataSub(g, ubacLagr, 'ubac', 'ubac', 1);
toc
